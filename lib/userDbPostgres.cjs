...
const userDb = {
  ...
  async createUser(userData) {
    try {
      ...
      const result = await query(
        ...
        [userData.email.toLowerCase().trim(), userData.name.trim(), passwordHash, false]
      );
  ...
  async verifyEmail(token) {
    try {
      console.log('🔍 verifyEmail: Attempting to verify token:', { tokenLength: token?.length, token: token?.substring(0, 8) + '...' });
      ...
      if (error.message?.includes('timeout') || error.message?.includes('Connection terminated')) {
        try {
          console.log('Timeout detected, checking if email was actually verified...');
          ...
          console.error('Error checking verification status:', checkError.message);
        }
      }
