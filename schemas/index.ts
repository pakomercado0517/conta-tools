...
export const ContractDataSchema = z
  .object({
    ...
    representantePrestador: z
      .string()
      .max(100, "Máximo 100 caracteres")
      .optional()
      .transform((val) => (val === "" ? undefined : val)),

    representanteCliente: z
      .string()
      .max(100, "Máximo 100 caracteres")
      .optional()
      .transform((val) => (val === "" ? undefined : val)),

    domicilioPrestador: z
      .string()
    ...
    clabeInterbancaria: z
      .string()
      .regex(/^\d{18}$/, "CLABE debe tener exactamente 18 dígitos")
      .optional()
      .transform((val) => (val === "" ? undefined : val)),

    usarNombreVendedorComoTitular: z.boolean().default(false),
    ...
    imagenFirmaVendedor: z.string().nullable().optional(),

    imagenFirmaComprador: z.string().nullable().optional(),
  })
  ...
  .refine(
    (data) => {
      ...
      if (data.fechaTermino === "otro") {
        return (
          data.fechaTerminoTexto && data.fechaTerminoTexto.trim().length > 0
        );
      }
      return true;
    },
...
export const PaybackDataSchema = z.object({
  inversion: z.number().min(0.01, "La inversión debe ser mayor a 0"),

  flujoEfectivoAnual: z
    .number()
