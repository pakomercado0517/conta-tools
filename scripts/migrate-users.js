...
async function migrateUsers() {
  ...
  try {
    ...
    console.log(
      `👥 Encontrados ${usersData.users.length} usuarios para migrar`
    );
    ...
    for (const jsonUser of usersData.users) {
      try {
        ...
        if (existingUser) {
          console.log(
            `⏭️  Usuario ${jsonUser.email} ya existe en PostgreSQL - omitiendo`
          );
          skippedCount++;
          continue;
        }
        ...
        `,
          [
            ...
            jsonUser.passwordResetExpires
              ? new Date(jsonUser.passwordResetExpires)
              : null,
        ...
        console.log(
          `✅ Usuario migrado: ${result.rows[0].email} (ID: ${result.rows[0].id})`
        );
        migratedCount++;
      } catch (userError) {
        console.error(
          `❌ Error migrando usuario ${jsonUser.email}:`,
          userError.message
        );
        errorCount++;
      }
    ...
    if (migratedCount > 0) {
      ...
      console.log(
        "💡 Tip: Puedes eliminar el archivo users.json ahora que los datos están en PostgreSQL"
      );
    }
  } catch (error) {
